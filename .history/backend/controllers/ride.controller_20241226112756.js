...
const createRide = async (request, response, next) => {
    ...
    try {
        ...
        const destCoordinate = await getAdressCoordinateService(destination);

        const captainsInRadius = await getCaptainInRadiusService(pickupCoordinate,destCoordinate,4); //ride within 4 Km radius
    } catch (error) {
