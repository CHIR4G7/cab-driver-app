...
const DriverFound = () => {
     ...
     const { confirmRide, setConfirmRide, waitingForDriver, setWaitingForDriver,driverFound,setDriverFound } = useContext(RideDataContext)
      ...
      return (
          <div className={`${driverFound ? '' : 'hidden'} w-full bg-white absolute bottom-0 flex flex-col items-center rounded-xl p-2 pb-4 `}>
              {/* <span className='absolute left-3 top-4' onClick={(e) => {
                  e.stopPropagation()
                  setDriverFound((prev) => !prev)
              }}><FaArrowLeft /></span> */}
              <span className='font-medium text-2xl'>Meet at the PickUp Location</span>
              {/* <div>
                  <Rings
                      visible={true}
                      height="100"
