  ...
  const handleActivityChange = ()=>{
    if(captain){
      if(captain.status==='Inactive'){
        sendMessage('change-activity',{captainId:captain._id,user,status:"Active"})
      }else{
        sendMessage('change-activity',{captainId:captain._id,user,status:"Inactive"})
      }
    }
  }
  useEffect(()=>{
  ...
  return (
    <div>
      <div className='absolute top-0 text-xl font-semibold bg-white text-black w-full flex flex-row justify-between px-2 h-11 pt-1'>
        ...
        <button className='w-20 h-9 border-2 border-black text-black rounded-md' onClick={handleActivityChange}>{captain.status}</button>
      </div>
