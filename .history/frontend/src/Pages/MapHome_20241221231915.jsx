...
const MapHome = () => {
  ...
  const [booking,setBooking] = useState(true) //while selecting cab,auto,mot and their fares to be displayed
    ...
    <div>
      ...
      <div ref={panelRef} className={`absolute bg-white w-full flex flex-col gap-3 p-2  pl-6 pb-3 ${selecting ? 'top-0 mt-4 rounded-xl h-screen' : 'bottom-0 rounded-3xl'} ${booking ? 'hidden' : ''}`} onClick={() => setSelecting(true)}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-1 left-6 ' >
          ...
          <button className='border-2 border-black text-white bg-black w-5/6 p-2 rounded-lg flex flex-row items-center justify-center mt-4'>Book a Ride</button>
        </form>
