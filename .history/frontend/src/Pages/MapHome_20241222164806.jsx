...
const MapHome = () => {
  ...
  // const [confirmRide,setConfirmRide] = useState(true)
  const {} = useContext(RideDataContext)
  ...
  return (
    <div>
      ...
      <div ref={panelRef} className={`absolute bg-white w-full flex flex-col gap-3 p-2  pl-6 pb-3 ${selecting ? 'top-0 mt-4 rounded-xl h-screen' : 'bottom-0 rounded-3xl pb-6'} ${booking ? 'hidden' : ''} ${confirmRide ? 'hidden' : ''}`} onClick={() => setSelecting(true)}>
      ...
      {/* <VehicleShowPanel selecting={selecting} booking={booking} setBooking={setBooking} setSelecting={setSelecting} setConfirmRide={setConfirmRide}/> */}
