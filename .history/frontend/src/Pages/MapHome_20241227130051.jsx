...
const MapHome = () => {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setRide({...ride,pickup:pickupRef.current.value,destination:destinationRef.current.value})
      setLoading((prev)=>!prev)
  ...
  const fetchSuggestions = async (query) => {
    ...
    try {
      // API call
      const url = `${import.meta.env.VITE_SERVER_BASE_URL}${apiRoutes.getAutoCompleteSuggestions.path}`
  ...
  return (
    <div>
     ...
     {fares!==null ?  <VehicleShowPanel fares={fares} ride={ride} setRide={setRide}/> : <></>}
